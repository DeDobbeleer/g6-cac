...
### 5.6 Name-to-ID Resolution (Apply Phase)

During the **validate** and **plan** phases, all cross-references use **resource names** (e.g., `routing_policy: rp-default`). This is because:
- Templates represent **desired state** (resources don't exist yet)
- IDs are generated by Director **on creation**
- Human-readable names are used in YAML

During the **apply** phase, names must be translated to Director IDs before API calls.

#### 5.6.1 Resolution Process

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  APPLY PHASE: Name-to-ID Translation                                        │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Step 1: Query Director API for existing resources                          │
│    GET /routingpolicies                                                     │
│    Response: [{"policy_name": "rp-default", "_id": "586cc3ed..."}, ...]      │
│                                                                             │
│  Step 2: Build name→ID lookup table                                         │
│    lookup = {                                                               │
│      "routing_policies": {"rp-default": "586cc3ed...", ...},                │
│      "enrichment_policies": {"ep-threat": "57591a2c...", ...},              │
│      ...                                                                    │
│    }                                                                        │
│                                                                             │
│  Step 3: Transform payload before API call                                  │
│    Template:           API Payload:                                         │
│    routing_policy:     routing_policy:                                      │
│      "rp-default"  →     "586cc3ed..."                                      │
│                                                                             │
│  Step 4: Send to Director API with real IDs                                 │
│    POST /processingpolicy                                                   │
│    Body: {"policy_name": "pp-test", "routing_policy": "586cc3ed..."}        │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### 5.6.2 Reference Mapping

| Source Resource | Target Resource | Template Field | API Field | Translation |
|-----------------|-----------------|----------------|-----------|-------------|
| ProcessingPolicy | RoutingPolicy | `routing_policy` | `routing_policy` | Name → ID |
| ProcessingPolicy | EnrichmentPolicy | `enrichment_policy` | `enrich_policy` | Name → ID |
| ProcessingPolicy | NormalizationPolicy | `normalization_policy` | `norm_policy` | Name → Name* |
| RoutingPolicy | Repo | `catch_all` | `catch_all` | Name → Name* |
| RoutingPolicy | Repo | `criteria[].repo` | `repo` | Name → Name* |

*No translation needed - these use names in both template and API.

#### 5.6.3 Resolution Order

Resources must be resolved in dependency order to ensure IDs are available:

```
1. Repos (created first, names used directly)
2. RoutingPolicies (created, IDs stored for lookup)
3. NormalizationPolicies (created, names used directly)
4. EnrichmentPolicies (created, IDs stored for lookup)
5. ProcessingPolicies (created using lookups from steps 2-4)
```

#### 5.6.4 Handling New Resources

For **new resources** being created during apply:
1. Create resource via POST
2. Extract `_id` from response
3. Add to lookup table immediately
4. Subsequent references can use the new ID

```python
# Example: Creating RP then referencing it in PP
rp_response = post("/routingpolicies", {"policy_name": "rp-new", ...})
lookup["routing_policies"]["rp-new"] = rp_response["_id"]

# Later in same apply session
pp_payload = {
    "policy_name": "pp-new",
    "routing_policy": lookup["routing_policies"]["rp-new"]  # Uses new ID
}
post("/processingpolicy", pp_payload)
```

#### 5.6.5 Validation vs Apply Difference

| Phase | Reference Format | Network Required | Why |
|-------|-----------------|------------------|-----|
| **Validate** | Names only | ❌ No | Offline check, IDs unknown |
| **Plan** | Names only | ❌ No | Offline diff, IDs unknown |
| **Apply** | IDs (via lookup) | ✅ Yes | Must use Director IDs |
