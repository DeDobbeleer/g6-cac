...
1. [Overview](#1-overview)
   ...
   - [1.2 Validation Prerequisites](#12-validation-prerequisites)
   - [1.3 4 Validation Levels](#13-4-validation-levels)
   - [1.4 Offline Validation vs Apply Phase](#14-offline-validation-vs-apply-phase)
...
│  ├── ProcessingPolicy → RoutingPolicy (by name)                            │
│  ├── ProcessingPolicy → NormalizationPolicy (by name)                      │
│  ├── ProcessingPolicy → EnrichmentPolicy (by name)                         │
│  ├── RoutingPolicy → Repo (catch_all, criteria)                            │
│  └── Deployment order calculation                                          │
└─────────────────────────────────────────────────────────────────────────────┘

**IMPORTANT**: All cross-references are validated by NAME, not ID.
IDs are generated by Director on resource creation and are unknown during
offline validation. See [Section 1.4](#14-offline-validation-vs-apply-phase).
```

### 1.4 Offline Validation vs Apply Phase

**Critical Concept**: The validation phase works on **desired state** (templates) without any API calls. At this stage:

1. **Resources do not exist yet** in Director (desired state)
2. **IDs are unknown** (generated by Director on creation)
3. **All references use NAMES** (human-readable, from YAML)
4. **Validation is 100% offline** (no network calls)

**Name-to-ID Translation** happens during the apply phase:
...
│      routing_policy: rp-default        ← Reference by NAME                  │
│      normalization_policy: _logpoint   ← Reference by NAME                  │
│                                                                             │
│  Validation:                                                                │
│    ✓ Check rp-default exists (by name)                                      │
│    ✓ Check _logpoint exists (by name)                                       │
│    ✗ Cannot check IDs (unknown yet)                                         │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────────────────────────┤
│  Step 1: Query Director to build name→ID map                                │
│    GET /routingpolicies → [{"policy_name": "rp-default", "_id": "586cc..."}] │
│                                                                             │
│  Step 2: Transform payload for API                                          │
│    routing_policy: "rp-default"  →  routing_policy: "586cc3ed..."           │
│                                                                             │
│  Step 3: Send to Director API                                               │
│    POST /processingpolicy with real IDs                                     │
└─────────────────────────────────────────────────────────────────────────────┘
```

**Reference Mapping**:

| From | To | In Template (validation) | In API Payload (apply) |
|------|----|-------------------------|------------------------|
| ProcessingPolicy | RoutingPolicy | `routing_policy: "rp-default"` | `routing_policy: "586cc3ed..."` |
| ProcessingPolicy | NormalizationPolicy | `normalization_policy: "_logpoint"` | `norm_policy: "_logpoint"` (name) |
| ProcessingPolicy | EnrichmentPolicy | `enrichment_policy: "ep-threat"` | `enrich_policy: "57591a2c..."` |
| RoutingPolicy | Repo (catch_all) | `catch_all: "repo-secu"` | `catch_all: "repo-secu"` (name) |
| RoutingPolicy | Repo (criteria) | `repo: "repo-secu"` | `repo: "repo-secu"` (name) |

**Key Points**:
- Validation uses **names** (what humans write in YAML)
- API uses **IDs** (what Director generates)
- Translation happens **during apply** via API queries
- Repos and Normalization Policies use **names in both** (no ID translation needed)
...
### 6.1 Name-Based Validation (Not ID-Based)

**Critical Rule**: All cross-references in Level 4 are validated by **RESOURCE NAME**, not by ID.

**Why?**
- Templates represent **desired state** (resources don't exist yet)
- IDs are generated by Director **on creation** (unknown during validation)
- Humans write **names** in YAML (e.g., `routing_policy: rp-default`)
- ID translation happens **during apply phase** via API calls
...
```
                           Repos
                            ...
                            ▼
                   ┌──────────────────┐
                   │  RoutingPolicy   │◄── catch_all: "repo-secu" (name)
                   │                  │◄── criteria[].repo: "repo-secu" (name)
                   └────────┬─────────┘
                            │
                            ▼
        ┌───────────────────────────────────────────┐
        │         ProcessingPolicy                  │
        │  ├── routing_policy: "rp-default"  (name) │
        │  ├── normalization_policy: "_logpoint"    │
        │  └── enrichment_policy: "ep-threat"(name) │
        └───────────────────────────────────────────┘
                            │
                            ▼
                      Devices (future)
...
**Note**: All references above use NAMES. During apply phase:
- `routing_policy: "rp-default"` → `routing_policy: "586cc3ed..."` (ID lookup via API)
- `enrichment_policy: "ep-threat"` → `enrich_policy: "57591a2c..."` (ID lookup via API)
- `catch_all: "repo-secu"` stays as name (repos use names in API)

### 6.3 Indexing for Fast Validation (Name-Based)
...
def _build_indexes(self) -> dict[str, set[str]]:
    ...
    CRITICAL: All validations use resource NAMES, not IDs.
    ...
    # NOTE: No ID indexes here - IDs are unknown in offline mode!
    # Name-to-ID translation happens during apply phase
    
    return indexes
```

### 6.4 Reference Validations (All by Name)

#### 6.4.1 ProcessingPolicy → RoutingPolicy (by name)

```python
for pp in resources.get("processing_policies", []):
    rp_ref = pp.get("routing_policy")
    if rp_ref and rp_ref != "None":
        # Validate by NAME, not ID (ID unknown in offline mode)
        if rp_ref not in indexes["routing_policies"]:
...
# 2. Transform: routing_policy: "rp-default" -> "586cc3ed..."
# 3. POST with real Director ID
```

#### 6.4.2 ProcessingPolicy → NormalizationPolicy (by name)

```python
np_ref = pp.get("normalization_policy")
if np_ref and np_ref != "None":
    # NormalizationPolicy uses names in both template and API
...
# Note: NormalizationPolicy uses name in API (not ID)
# No translation needed during apply
```

#### 6.4.3 ProcessingPolicy → EnrichmentPolicy (by name)

```python
ep_ref = pp.get("enrichment_policy")
if ep_ref and ep_ref != "None":
    # Validate by NAME, not ID (ID unknown in offline mode)
    if ep_ref not in indexes["enrichment_policies"]:
        errors.append(ValidationError(
            resource_type="processing_policies",
            resource_name=pp["policy_name"],
...
#### 6.4.4 RoutingPolicy → Repo (catch_all, by name)
...
# Note: Repos use names in both template and API
# No ID translation needed during apply
```

#### 6.4.5 RoutingPolicy → Repo (criteria, by name)
...
### 6.5 Apply Phase: Name-to-ID Translation (Future)

During the apply phase (not yet implemented), the system will:

1. **Query Director API** to build name→ID mappings:
   ```python
   # GET /routingpolicies
   response = [
       {"policy_name": "rp-default", "_id": "586cc3edd8aaa406f6fdc8e3"},
       {"policy_name": "rp-windows", "_id": "586cc3f0d8aaa406f6fdc8e4"},
   ]
   name_to_id = {r["policy_name"]: r["_id"] for r in response}
   ```

2. **Transform payloads** before sending to API:
   ```python
   ...
   api_payload = {
       "policy_name": "pp-default",
       "routing_policy": "586cc3ed...",       # ID
       "enrich_policy": "57591a2c...",        # ID
   }
   ```

3. **Resources that need ID translation**:
   | Resource | Template Field | API Field | Translation |
   |----------|---------------|-----------|-------------|
   | ProcessingPolicy | `routing_policy` | `routing_policy` | Name → ID |
   | ProcessingPolicy | `enrichment_policy` | `enrich_policy` | Name → ID |
   | ProcessingPolicy | `normalization_policy` | `norm_policy` | Name → Name (no change) |
   | RoutingPolicy | `catch_all` | `catch_all` | Name → Name (no change) |
   | RoutingPolicy | `criteria[].repo` | `repo` | Name → Name (no change) |
